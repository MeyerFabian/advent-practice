...
fn captcha2(vec: &Vec<u32>) {
    let halfLen = vec.len() / 2;

    let sum = vec.iter()
        .take(halfLen)
        .zip(vec.iter().cycle().skip(halfLen))
        .fold(0, |acc, (ai, bi)| if ai == bi { acc + 2 * ai } else { acc });
...
fn captcha1(vec: &Vec<u32>) {
    let sum = vec.iter()
        .zip(vec.iter().cycle().skip(1))
        .fold(0, |acc, (ai, bi)| if ai == bi { acc + ai } else { acc });
    println!("Captcha Code is: {}!", sum);
}
