...
// it instead uses a specific starting value as its "previous value"
// (as listed in your puzzle input).

// For example, suppose that for starting values,
// generator A uses 65, while generator B uses 8921.
...
// 16 bits of the third value match: 1110001101001010.
// Because of this one match, after processing these five pairs,
// the judge would have added only 1 to its total.
...
// and otherwise working through the same sequence of values as before
// until they find one.
...
// none of these values' lowest 16 bits match. Now,
...
// the judge would eventually find a total of 309 pairs that match in their lowest 16 bits.)
